...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const signUp = async (email: string, password: string, fullName: string, isAdmin: boolean) => {
    try {
      ...
      if (data.user) {
        // Step 2: Create a profile entry in the profiles table
        ...
        const { error: profileError } = await supabase
          ...
          .insert({
            ...
            is_admin: isAdmin,
            is_student: !isAdmin,
            admin_approved: !isAdmin // Auto-approve students, admins need approval
          });
          
        if (profileError) {
          console.error('Error creating profile:', profileError);
          // If profile creation fails, we should log the error but continue
          // We won't delete the auth user as that requires admin privileges
